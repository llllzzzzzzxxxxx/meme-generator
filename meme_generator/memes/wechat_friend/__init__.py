...
def wechat_friend(images: list[BuildImage], texts: list[str], args: Model):
    ...
    qr_img = qr_img.resize((595, 595))
    frame.paste(qr_img, (177, 390))

    block = BuildImage.new("RGBA", (159, 159), "white")
    frame.paste(block, (396, 618))

    avatar = avatar.convert("RGBA").resize((141, 141), keep_ratio=True).circle_corner(8)
    frame.paste(avatar, (405, 623), alpha=True)
    frame.paste(avatar, (131, 131), alpha = True)
...
add_meme(
    ...
    args_type=args_type,
    keywords=["恰个v"],
    date_created=datetime(2024, 11, 22),
